...
export default function ImpressaoPage(){
  ...
  const [files,setFiles]=useState<FileCfg[]>([]); const [busy,setBusy]=useState(false); const [openExtras,setOpenExtras]=useState<Record<string,boolean>>({}); const [openAdvanced,setOpenAdvanced]=useState<Record<string,boolean>>({});
      ...
      return <section className="file-card" key={f.id}>
        ...
        <div className="block"><h2>Como você quer imprimir?</h2>
          <div className="visual-choices">
            <button className={f.color==='bw'?'visual-choice active':'visual-choice'} onClick={()=>update(f.id,{color:'bw'})}>
              <span className="paper-preview bw-preview"><i/><i/><i/><i/></span><span><b>Preto e branco</b><small>Para textos e documentos</small></span>{f.color==='bw'&&<Check/>}
            </button>
            {hasColor&&<button className={f.color==='color'?'visual-choice active':'visual-choice'} onClick={()=>update(f.id,{color:'color'})}>
              <span className="paper-preview color-preview"><i/><i/><i/><i/></span><span><b>Colorido</b><small>Para fotos e imagens</small></span>{f.color==='color'&&<Check/>}
            </button>}
          </div>
        </div>

        <div className="advanced"><button className="advanced-toggle" onClick={()=>setOpenAdvanced(v=>({...v,[f.id]:!v[f.id]}))}><span>Mais opções de impressão</span><ChevronDown className={openAdvanced[f.id]?'up':''}/></button>
          {openAdvanced[f.id]&&<div className="advanced-content"><div><h3>Impressão</h3><div className="choices two">
          ...
          </div></div><div className="advanced-copies"><h3>Quantidade de cópias</h3><div className="qty"><button onClick={()=>update(f.id,{copies:Math.max(1,f.copies-1)})}><Minus/></button><strong>{f.copies}</strong><button onClick={()=>update(f.id,{copies:f.copies+1})}><Plus/></button></div></div></div>}
        </div>

        <div className="block"><h2>Tipo de folha</h2>
          {papers.length>1?<div className="visual-choices paper-visual">{papers.map(p=><button key={p.id} className={f.paperId===p.id?'visual-choice active':'visual-choice'} onClick={()=>update(f.id,{paperId:p.id})}><span className="sheet-icon">A4</span><span><b>{p.name}</b><small>{p.size}</small></span>{f.paperId===p.id&&<Check/>}</button>)}</div>
          :<div className="paper-fixed"><span className="sheet-icon small">A4</span><span><b>{paper?.name||'Papel padrão'}</b><small>{paper?.size||'Folha padrão'}</small></span><Check size={17}/></div>}
        </div>

        {finishing.length>0&&<div className="block finish-block"><div className="block-title"><div><h2>Quer algum acabamento?</h2><small>Escolha se quer encadernar ou plastificar. A quantidade é calculada automaticamente.</small></div></div>
          <div className="choices finish-types">
            <button className={!finish?'finish-choice active':'finish-choice'} onClick={()=>clearFinish(f)}><span className="finish-icon none">✓</span><b>Nenhum</b><small>Somente a impressão</small>{!finish&&<Check/>}</button>
            {lamination.length>0&&<button className={finishMode==='lamination'?'finish-choice active':'finish-choice'} onClick={()=>chooseFinish(f,lamination[0].id)}><span className="finish-icon">▣</span><b>Plastificar</b><small>Escolha o documento</small>{finishMode==='lamination'&&<Check/>}</button>}
            {binding.length>0&&<button className={finishMode==='binding'?'finish-choice active':'finish-choice'} onClick={()=>{if(applicable)chooseFinish(f,applicable.id);}} disabled={!applicable}><span className="finish-icon">▤</span><b>Encadernar</b><small>{applicable?'Calculado automaticamente':'Não disponível para esta quantidade'}</small>{finishMode==='binding'&&<Check/>}</button>}
          </div>

          {finishMode==='lamination'&&<div className="finish-options"><div className="finish-label">O que você quer plastificar?</div>{lamination.map(s=><button key={s.id} className={finish?.id===s.id?'finish-option active':'finish-option'} onClick={()=>chooseFinish(f,s.id)}><span><b>{s.name.replace(/^Plastificação\s*—\s*/i,'')}</b><small>{s.description||'Serviço de plastificação'}</small></span><strong>{money(s.price)}{s.charge_type==='per_sheet'?' / folha':' / unidade'}</strong>{finish?.id===s.id&&<Check/>}</button>)}</div>}
  ...
  <style jsx global>{`
    ...
    .block{padding-top:17px}.block h2{font-size:14px;margin:0 0 9px}.block h2+small{color:#777}.choices{display:grid;gap:8px}.choices.two{grid-template-columns:1fr 1fr}.visual-choices{display:grid;grid-template-columns:1fr 1fr;gap:10px}.visual-choice{position:relative;border:1px solid #dcdad5;background:#fff;border-radius:13px;padding:11px;text-align:left;display:flex;align-items:center;gap:11px;min-height:82px;cursor:pointer}.visual-choice.active{border:2px solid #ffc400;background:#fffbea}.visual-choice>span:nth-child(2){display:grid;gap:3px}.visual-choice b{font-size:13px}.visual-choice small{font-size:10px;color:#777}.visual-choice>svg{margin-left:auto;color:#7a5b00;flex:none}.paper-visual{grid-template-columns:repeat(auto-fit,minmax(180px,1fr))}.paper-visual .visual-choice{min-height:64px}.paper-preview{width:48px;height:60px;border:1px solid #cfcfcf;border-radius:5px;display:grid;grid-template-columns:repeat(2,1fr);gap:4px;padding:7px;flex:none;background:#fff}.paper-preview i{display:block;border-radius:2px}.bw-preview i{background:#888}.color-preview i:nth-child(1){background:#ef5b5b}.color-preview i:nth-child(2){background:#4b86e8}.color-preview i:nth-child(3){background:#56a764}.color-preview i:nth-child(4){background:#f0b83d}.sheet-icon{width:43px;height:54px;border:1px solid #ccc;border-radius:5px;display:grid;place-items:center;font-size:10px;font-weight:900;background:#fff;flex:none}.sheet-icon.small{width:32px;height:40px;font-size:8px}.paper-fixed{margin-top:0;background:#f6f6f3;border-radius:10px;padding:11px 13px;display:flex;gap:10px;align-items:center}.paper-fixed>span:nth-child(2){display:grid;gap:2px;flex:1}.paper-fixed b{font-size:11px}.paper-fixed small{font-size:10px;color:#777}.advanced{border-top:1px solid #eee;margin-top:16px;padding-top:8px}.advanced-toggle,.extras-toggle{width:100%;border:0;background:transparent;display:flex;justify-content:space-between;align-items:center;padding:8px 0;font:900 11px Inter;cursor:pointer}.advanced-toggle svg,.extras-toggle svg{width:16px}.advanced-toggle .up,.extras-toggle .up{transform:rotate(180deg)}.advanced-content{display:grid;grid-template-columns:1fr auto;gap:15px;background:#f7f7f4;border-radius:10px;padding:12px}.advanced-content h3{font-size:10px;margin:0 0 7px}.advanced-copies{display:grid;align-content:start}.choice{position:relative;border:1px solid #dcdad5;background:#fff;border-radius:11px;padding:13px 40px 13px 13px;text-align:left;min-height:62px;cursor:pointer}.choice.active{border:2px solid #ffc400;background:#fffbea}.choice:disabled{opacity:.5;cursor:not-allowed}.choice b{display:block;font-size:12px}.choice small{display:block;margin-top:3px;color:#777;font-size:10px;line-height:1.35}.choice svg{position:absolute;right:12px;top:50%;transform:translateY(-50%);color:#7a5b00}.paper-choices{grid-template-columns:repeat(auto-fit,minmax(160px,1fr))}
    ...
    .finish-block{border-top:1px solid #eee;margin-top:18px}.block-title{display:flex;justify-content:space-between;gap:10px}.block-title h2{margin:0}.block-title small{font-size:10px;color:#777}.finish-types{grid-template-columns:repeat(3,1fr)}.finish-choice{position:relative;border:1px solid #dcdad5;background:#fff;border-radius:12px;padding:12px 34px 12px 11px;text-align:left;min-height:78px;display:grid;grid-template-columns:32px 1fr;gap:2px 8px;align-items:center;cursor:pointer}.finish-choice.active{border:2px solid #ffc400;background:#fffbea}.finish-choice:disabled{opacity:.5;cursor:not-allowed}.finish-choice b{font-size:12px}.finish-choice small{font-size:9px;color:#777;grid-column:2}.finish-choice>svg{position:absolute;right:10px;top:50%;transform:translateY(-50%);color:#7a5b00}.finish-icon{grid-row:1 / span 2;width:32px;height:32px;border-radius:8px;background:#f1f1ee;display:grid;place-items:center;font-size:15px;font-weight:900}.finish-choice.active .finish-icon{background:#ffc400}.finish-options{margin-top:9px;padding:10px;background:#f7f7f4;border-radius:10px}.finish-label{font-size:9px;font-weight:900;color:#666;margin-bottom:7px;text-transform:uppercase;letter-spacing:.08em}.finish-option{width:100%;border:1px solid #ddd;background:#fff;border-radius:9px;padding:10px;text-align:left;display:flex;gap:8px;align-items:center;margin-top:6px;cursor:pointer}.finish-option.active{border:2px solid #ffc400;background:#fffbea}.finish-option span{display:grid;gap:2px;flex:1}.finish-option b{font-size:11px}.finish-option small{font-size:9px;color:#777}.finish-option strong{font-size:11px;white-space:nowrap}.auto-rule{margin-top:9px;display:flex;gap:8px;align-items:flex-start;padding:10px;background:#f6f6f3;border-radius:9px;color:#555;font-size:10px;line-height:1.4}.auto-rule svg{flex:none}.auto-rule.warning{background:#fff4dc;color:#765c00}
    ...
    @media(min-width:701px){
      ...
      .hero span{max-width:820px}
      .mark{width:52px;height:52px;border-radius:13px}
      .upload-box{padding:18px;grid-template-columns:auto 1fr auto;grid-template-rows:auto auto;column-gap:14px;text-align:left;place-items:center start}
      .upload-icon{grid-row:1 / span 2}
      .upload-box strong{align-self:end}
      .upload-box span{align-self:start}
      .upload-box button{grid-column:3;grid-row:1 / span 2}
      .files{gap:12px;margin-top:12px}
      .file-card{padding:20px 22px}
      .file-head{padding-bottom:12px}
      .block{padding-top:13px}
      .visual-choices{gap:8px}
      .visual-choice{min-height:70px;padding:9px 11px}
      .paper-visual .visual-choice{min-height:58px}
      .advanced{margin-top:12px;padding-top:5px}
      .finish-block{margin-top:14px}
      .finish-choice{min-height:68px;padding:9px 34px 9px 11px}
      .extras{margin-top:12px;padding-top:9px}
      .file-total{margin-top:12px;padding-top:10px}
      .summary{margin-top:12px}
    }
    @media(max-width:700px){.hero{align-items:flex-start}.hero h1{font-size:39px}.visual-choices,.choices.two,.finish-types{grid-template-columns:1fr}.advanced-content{grid-template-columns:1fr}.summary{flex-wrap:wrap}.summary button{width:100%}.file-total{display:grid;grid-template-columns:1fr 1fr}.main-total{margin-left:0;text-align:left;grid-column:1/-1}.paper-choices{grid-template-columns:1fr 1fr}}
    @media(max-width:430px){.visual-choice{min-height:72px}.print-shell{width:calc(100% - 16px)}.hero h1{font-size:34px}.hero span{font-size:12px}.paper-choices{grid-template-columns:1fr}.file-card{padding:14px}}
  `}</style></main>
}
