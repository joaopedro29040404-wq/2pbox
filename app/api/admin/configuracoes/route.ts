...
export async function PUT(request: Request) {
  ...
  const operations = {
    business_hours: hours, business_days: days, opens_at: firstDay?.open ?? null, closes_at: firstDay?.close ?? null,
    shipping_mode: text(body.shippingMode, current.shippingMode), pickup_mode: text(body.pickupMode, current.pickupMode),
    service_fee_percent: bounded(body.serviceFeePercent, 0, 100, current.serviceFeePercent), service_fee_fixed: bounded(body.serviceFeeFixed, 0, 10000, current.serviceFeeFixed),
    min_order_total: bounded(body.minOrderTotal, 0, 100000, current.minOrderTotal), free_shipping_from: body.freeShippingFrom == null || body.freeShippingFrom === '' ? null : bounded(body.freeShippingFrom, 0, 100000, 0),
    address_line: text(body.address?.line, current.address.line), address_number: text(body.address?.number, current.address.number), address_complement: text(body.address?.complement, current.address.complement),
    address_district: text(body.address?.district, current.address.district), address_city: text(body.address?.city, current.address.city), address_state: text(body.address?.state, current.address.state).toUpperCase().slice(0, 2),
    address_zip: text(body.address?.zip, current.address.zip), address_place_id: text(body.address?.placeId, current.address.placeId || '') || null,
    address_lat: coordinate(body.address?.lat), address_lng: coordinate(body.address?.lng),
    delivery_pickup_enabled: bool(body.pickupEnabled, current.pickupEnabled), delivery_own_enabled: bool(body.ownDeliveryEnabled, current.ownDeliveryEnabled),
    ...
    delivery_express_price_table: normalizePriceTable(body.expressPriceTable), delivery_express_max_km: bounded(body.expressMaxKm, 0.5, 200, current.expressMaxKm),
    ...
    delivery_max_km: bounded(body.maxKm, 0.5, 200, current.maxKm), delivery_price_table: normalizePriceTable(body.priceTable),
  ...
  try {
    ...
    if (!/column|schema cache/i.test(message)) return NextResponse.json({ error: message || 'Não foi possível salvar as configurações.' }, { status: 500 });
    try {
      await persist(current.id, legacy);
      invalidateStoreOperations();
      return NextResponse.json({ ok: true, operationsSchema: false, warning: 'As configurações de operação exigem a migration 20260913_store_operations.sql. Apenas os dados básicos foram salvos.', operations: await readStoreOperations({ fresh: true }) });
    } catch (fallbackError) {
...
async function persist(id: string | null, payload: Record<string, unknown>) {
  if (id) {
    await supabaseRest(`store_settings?id=eq.${encodeURIComponent(id)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify(payload) });
    return;
  }
  await supabaseRest('store_settings', { method: 'POST', headers: { Prefer: 'return=minimal' }, body: JSON.stringify([payload]) });
}
