...
export async function GET(request: Request) {
  ...
  const funnelStages = [
    ...
    ...stage,
    percent: visitors.size ? round((stage.count / visitors.size) * 100) : 0,
    nextConversion: index < list.length - 1 && stage.count ? round((list[index + 1].count / stage.count) * 100) : null,
    abandonment: index < list.length - 1 ? Math.max(0, stage.count - list[index + 1].count) : 0,
  }));

  const sessionSource = new Map<string, string>();
  for (const event of pageViews) {
    if (!sessionSource.has(event.session_id)) sessionSource.set(event.session_id, sourceLabel(event.utm_source));
  }
  ...
  const ensureTraffic = (source: string) => {
    if (!trafficMap.has(source)) trafficMap.set(source, { source, visits: new Set(), carts: new Set(), checkouts: new Set(), orders: 0, revenue: 0 });
    return trafficMap.get(source)!;
  };
  ...
  const { data: itemRows } = paidOrders.length
    ? await client.from('order_items').select('order_id,product_id,product_name,quantity,total').in('order_id', paidOrders.map((order) => order.id)).range(0, 19999)
    : { data: [] as any[] };
  ...
  const ensureProduct = (id: string) => {
    if (!productMap.has(id)) productMap.set(id, { id, name: names.get(id) || 'Produto', views: 0, carts: 0, sales: 0, revenue: 0 });
  ...
  return NextResponse.json({
    ...
    cards: {
      ...
      payments: paymentSessions.size,
      approvedOrders: paidOrders.length,
      orders: realOrders.length,
      canceledOrders: canceledOrders.length,
      revenue: round(revenue),
      averageTicket: paidOrders.length ? round(revenue / paidOrders.length) : 0,
      conversionRate,
      productsSold: items.reduce((sum, item) => sum + Number(item.quantity || 0), 0),
    },
    evolution,
    funnel: funnelStages,
    traffic,
    products,
    abandoned: { carts: cartCreatedSessions, abandoned: abandonedSessions.length, rate: abandonedRate, value: round(abandonedValue) },
    devices,
    hours,
    sales: { orders: realOrders.length, approved: paidOrders.length, canceled: canceledOrders.length, revenue: round(revenue), averageTicket: paidOrders.length ? round(revenue / paidOrders.length) : 0, productsSold: items.reduce((sum, item) => sum + Number(item.quantity || 0), 0) },
    testOrders,
  });
}
