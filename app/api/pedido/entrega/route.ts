...
export async function POST(request: Request) {
  ...
  if (!email || !orderEmail || email !== orderEmail) return NextResponse.json({ error: 'Não foi possível confirmar o pedido.' }, { status: 403 });
  if (String(order.payment_status || '') === 'paid') return NextResponse.json({ error: 'Este pedido já está pago.' }, { status: 409 });
  ...
  if (provider === 'own' || provider === 'express' || provider === 'app') {
  ...
  const payload: Record<string, unknown> = {
    ...
    delivery_cycle_start: cycleStartFor(new Date(), operations.cycleHour).toISOString(), items_subtotal: subtotal, total, updated_at: new Date().toISOString(),
  };
