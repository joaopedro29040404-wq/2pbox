...
const statusPriority: Record<string, number> = { processed: 100, approved: 100, accredited: 100, authorized: 80, in_process: 60, pending: 40, failed: 20, rejected: 20, canceled: 10, cancelled: 10, refunded: 5, charged_back: 5 };
...
function normalizePaymentFromOrder(order: any, fallbackOrderId: string) {
  ...
  return { ...payment, id: payment?.id || payment?.reference_id || order?.id, external_reference: String(order?.external_reference || fallbackOrderId), status: statusPriority[orderStatus] ? orderStatus : (paymentStatus || orderStatus || 'pending'), status_detail: payment?.status_detail || order?.status_detail || null, order_status: orderStatus || null, order_status_detail: order?.status_detail || null };
}
...
async function findCurrentMpOrder(accessToken: string, externalReference: string) {
  ...
  matching.sort((a: any, b: any) => (statusPriority[String(b?.status || '').toLowerCase()] || 0) - (statusPriority[String(a?.status || '').toLowerCase()] || 0) || (new Date(String(b?.last_updated_date || b?.created_date || 0)).getTime() - new Date(String(a?.last_updated_date || a?.created_date || 0)).getTime()));
...
async function findCurrentLegacyPayment(accessToken: string, externalReference: string, paymentId: string) {
  ...
  payments.sort((a: any, b: any) => (statusPriority[String(b?.status || '').toLowerCase()] || 0) - (statusPriority[String(a?.status || '').toLowerCase()] || 0) || (new Date(String(b?.date_last_updated || b?.date_created || 0)).getTime() - new Date(String(a?.date_last_updated || a?.date_created || 0)).getTime()));
  return payments[0] || null;
}

async function readOrder(admin: any, orderId: string, email: string) {
  const { data, error } = await admin.from('orders').select('id,customer_name,customer_phone,customer_email,delivery_type,delivery_address,notes,status,payment_status,total,created_at,payment_id,payment_status_detail,payment_updated_at').eq('id', orderId).ilike('customer_email', email).maybeSingle();
...
export async function GET(request: Request) {
  try {
    ...
    if (requestedOrderId) orderQuery = orderQuery.eq('id', requestedOrderId); else orderQuery = orderQuery.order('created_at', { ascending: false }).limit(1);
    ...
    if (localStatus === 'confirmed' || localPaymentStatus === 'approved') {
      ...
      return NextResponse.json({ order: { ...result.order, status: localStatus === 'confirmed' ? 'confirmed' : result.order.status, payment_status: 'approved' }, items: result.items, payment: result.order.payment_id ? { id: String(result.order.payment_id), status: 'approved', statusDetail: result.order.payment_status_detail || 'accredited' } : null, source: 'supabase' }, { headers: { 'Cache-Control': 'no-store, max-age=0, s-maxage=0' } });
    }
    ...
    if (accessToken) {
      try {
        ...
        if (mpPayment && mpPayment.external_reference === orderId) {
          await syncOrderPayment(orderId, mpPayment);
        } else {
          const legacyPayment = await findCurrentLegacyPayment(accessToken, orderId, String(currentOrder.payment_id || '').trim());
          if (legacyPayment && String(legacyPayment.external_reference || '').trim() === orderId) await syncOrderPayment(orderId, legacyPayment);
        }
      } catch (error) { console.error('Public Mercado Pago reconciliation error:', error); }
    }
    ...
    return NextResponse.json({ order: { ...result.order, payment_status: canonicalPaymentStatus, status: canonicalOrderStatus }, items: result.items, payment: result.order.payment_id ? { id: String(result.order.payment_id), status: canonicalPaymentStatus, statusDetail: result.order.payment_status_detail || null } : null, source: accessToken ? 'supabase+mercadopago' : 'supabase' }, { headers: { 'Cache-Control': 'no-store, max-age=0, s-maxage=0' } });
  } catch (error) {
