...
export async function POST(request: Request) {
  ...
  try {
    ...
    const orderBody = {
      ...
      transactions: { payments: [{ amount: amount.toFixed(2), payment_method: { id: paymentMethodId, type: paymentMethodType, token: String(formData.token), installments: Number(formData.installments || 1) } }] }
    };
    const headers: Record<string, string> = { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json', 'X-Idempotency-Key': crypto.randomUUID() };
