...
function validateWebhookSignature(request: Request, dataId: string) {
  ...
  const parts = Object.fromEntries(xSignature.split(',').map((part) => part.split('=').map((value) => value.trim())).filter(([key, value]) => key && value)) as Record<string, string>;
  ...
  try { return expected.length === v1.length && timingSafeEqual(Buffer.from(expected), Buffer.from(v1)); } catch { return false; }
}
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`https://api.mercadopago.com${endpoint}`, { headers: { Authorization: `Bearer ${accessToken}` }, cache: 'no-store' });
    ...
    if (type === 'order' || resource?.type === 'online') {
      ...
      if (orderId && payment) await syncOrderPayment(orderId, { ...payment, id: payment?.id || resource?.id, external_reference: orderId });
    } else {
      const orderId = String(resource?.external_reference || '').trim();
