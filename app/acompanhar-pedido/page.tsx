  ...
  return (
    <main className="track-page">
      ...
      <header className="track-header">
        <div className="track-container track-header-inner">
          ...
          <nav className={menuOpen ? 'track-nav track-nav-open' : 'track-nav'}>
            ...
            <Link href="/acompanhar-pedido" onClick={() => setMenuOpen(false)}>Acompanhar pedido</Link>
          </nav>
          <div className="track-actions">
            <Link href="/conta" className="track-account">Entrar</Link>
            <Link href="/carrinho" className="track-cart"><ShoppingBag size={18} strokeWidth={1.9}/><span>Carrinho</span></Link>
            <button className="track-menu" onClick={() => setMenuOpen(v => !v)} aria-label="Abrir menu">{menuOpen ? <X size={21}/> : <Menu size={21}/>}</button>
          </div>
        </div>
      </header>
        ...
        <div className="track-container">
          <div className="track-heading">
            <div>
              ...
              <p className="track-lead">Consulte o andamento da sua compra informando o número do pedido e o e-mail usado no checkout.</p>
            </div>
          </div>
          ...
          <div className="track-layout">
            <aside className="track-side">
              ...
              <h2>Seu pedido,<br/><em>do começo ao fim.</em></h2>
              <p>Sem login. É só informar os dados usados na compra.</p>
            </aside>

            <div className="track-card">
              <div className="track-card-head">
                <div className="track-icon"><Search size={21} strokeWidth={2}/></div>
                <div><p>CONSULTA RÁPIDA</p><h2>Localize sua compra</h2></div>
              </div>
              <form onSubmit={submit} noValidate>
                ...
                <button type="submit">ACOMPANHAR PEDIDO <ArrowRight size={17}/></button>
              </form>
      ...
      <style jsx global>{`
        ...
        .track-topbar{height:34px;background:#111;color:#fff;display:flex;align-items:center;justify-content:center;gap:14px;font-size:10px;font-weight:800;letter-spacing:.25em}.track-topbar b{color:var(--yellow);font-size:9px}
        .track-header{height:86px;background:rgba(255,255,255,.97);backdrop-filter:blur(12px);border-bottom:1px solid var(--line);position:sticky;top:0;z-index:50}.track-header-inner{height:100%;display:flex;align-items:center;gap:38px}.track-brand{display:block;width:112px;height:70px;flex:none}.track-brand img{width:100%;height:100%;object-fit:contain}.track-nav{display:flex;align-items:center;gap:34px;margin-left:auto}.track-nav a,.track-account{font-size:11px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;text-decoration:none;color:#111}.track-nav a:hover,.track-account:hover{color:var(--gold)}.track-actions{display:flex;align-items:center;gap:20px}.track-account{white-space:nowrap}.track-cart{height:42px;display:flex;align-items:center;gap:9px;padding:0 14px;background:#111;color:#fff;text-decoration:none;border-radius:8px;font-size:11px;font-weight:800}.track-menu{display:none;width:42px;height:42px;background:#fff;border:1px solid #ddd;border-radius:8px}
