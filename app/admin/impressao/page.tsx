...
export default function AdminImpressao() {
  ...
  async function save(table: string, row: any) {
    ...
    for (const key of ['min_sheets', 'max_sheets', 'sort_order']) if (payload[key] != null) payload[key] = Number(payload[key]); if (payload.service_group !== 'binding') { payload.min_sheets = null; payload.max_sheets = null; } payload.selection_group = payload.service_group === 'other' ? null : 'finishing';
  ...
  return (
    <main className="pa">
      ...
      <section className="shell">
        ...
        <section className="card">
          <div className="title">
            ...
            <button className="primary" onClick={() => setServices(v => [...v, { id: crypto.randomUUID(), name: 'Novo serviço', description: '', charge_type: 'per_document', price: 0, active: true, sort_order: v.length, service_group: 'other', selection_group: null, min_sheets: '', max_sheets: '' }])}><Plus size={15} /> Adicionar serviço</button>
          </div>

          <div className="service-rules">
            <b>Regras para o cliente</b>
            <p>Encadernação e plastificação são <strong>acabamentos</strong>: o cliente escolhe apenas um deles. Na encadernação, o sistema escolhe automaticamente a faixa correta pelo número de folhas.</p>
          </div>
            ...
            {services.map(s => (
              <div className="service-card" key={s.id}>
                <div className="service-fields">
                  <label className="field"><span>Nome do serviço</span><input value={s.name} onChange={e => setServices(v => v.map(x => x.id === s.id ? { ...x, name: e.target.value } : x))} placeholder="Ex.: Encadernação 1 a 50 folhas" /></label>
                  <label className="field"><span>Tipo</span><select value={s.service_group || 'other'} onChange={e => setServices(v => v.map(x => x.id === s.id ? { ...x, service_group: e.target.value, min_sheets: e.target.value === 'binding' ? x.min_sheets : '', max_sheets: e.target.value === 'binding' ? x.max_sheets : '' } : x))}><option value="other">Outro serviço</option><option value="lamination">Plastificação</option><option value="binding">Encadernação</option></select></label>
                  <label className="field"><span>Descrição</span><input value={s.description || ''} onChange={e => setServices(v => v.map(x => x.id === s.id ? { ...x, description: e.target.value } : x))} placeholder="Ex.: Plastificação de documento" /></label>
                  ...
                  {s.service_group === 'binding' && <><label className="field"><span>De (folhas)</span><input type="number" min="1" value={s.min_sheets ?? ''} onChange={e => setServices(v => v.map(x => x.id === s.id ? { ...x, min_sheets: e.target.value } : x))} /></label><label className="field"><span>Até (folhas)</span><input type="number" min="1" value={s.max_sheets ?? ''} onChange={e => setServices(v => v.map(x => x.id === s.id ? { ...x, max_sheets: e.target.value } : x))} /></label></>}
      ...
      <style jsx global>{`
        ...
        .service-help{background:#f8f8f6;border-radius:11px;padding:13px;margin-bottom:13px}.service-help>b{font-size:10px}.service-help-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:9px}.service-help-grid div{background:#fff;border:1px solid #e4e3df;border-radius:8px;padding:9px;display:grid;gap:4px}.service-help-grid strong{font-size:9px}.service-help-grid span{line-height:1.4}.services-list{display:grid;gap:9px}.service-card{border:1px solid #e4e3df;border-radius:12px;padding:13px}.service-fields{display:grid;grid-template-columns:1.1fr .8fr 1.2fr .8fr .55fr auto;gap:10px;align-items:end}.service-rules{background:#fff9db;border:1px solid #f0d86b;border-radius:11px;padding:12px;margin-bottom:13px;font-size:10px}.service-rules p{margin:6px 0 0;color:#555;line-height:1.5}.service-actions{display:flex;gap:10px;align-items:center;margin-top:11px}.save-wide{border:0;background:#ffc400;border-radius:8px;padding:9px 12px;font:900 9px Inter;display:flex;align-items:center;gap:5px;cursor:pointer}.delete-text{border:0;background:transparent;color:#a33;font:800 9px Inter;display:flex;gap:5px;align-items:center;cursor:pointer}
