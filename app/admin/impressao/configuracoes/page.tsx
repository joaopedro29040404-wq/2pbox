...
import { ArrowLeft, HelpCircle, Plus, Save, Trash2, Printer } from 'lucide-react';
  ...
  return (
    <main className="pa">
      ...
      <section className="shell">
        <div className="top-row"><Link href="/admin" className="back"><ArrowLeft size={15} /> Painel</Link></div>
        <nav className="print-tabs" aria-label="Central de impressão">
          <Link href="/admin/impressao/pedidos" className="tab"><Printer size={15} /> Pedidos <span>principal</span></Link>
          <Link href="/admin/impressao/configuracoes" className="tab active"><Printer size={15} /> Configurações</Link>
        </nav>
        ...
        <header className="hero">
          ...
          <div>
            <p>IMPRESSÃO</p>
            <h1>Configurações de impressão</h1>
      ...
      <style jsx global>{`
        ...
        .top-row{display:flex;align-items:center;margin-bottom:16px}.back{display:inline-flex;gap:7px;align-items:center;color:#666;text-decoration:none;font:800 11px Inter}
        .print-tabs{display:flex;gap:4px;padding:4px;background:#ebeae5;border:1px solid #deddd7;border-radius:12px;margin-bottom:20px}.tab{display:flex;align-items:center;justify-content:center;gap:7px;padding:11px 15px;border-radius:9px;color:#666;text-decoration:none;font:900 10px Inter;flex:1}.tab.active{background:#111;color:#fff}.tab span{font-size:8px;opacity:.65}.hero{display:flex;gap:15px;align-items:center;margin-bottom:18px}.hero-icon{width:52px;height:52px;border-radius:15px;background:#ffc400;display:grid;place-items:center}.hero p{margin:0 0 5px;color:#9b7600;font-size:10px;font-weight:900;letter-spacing:.2em}.pa h1{margin:0;font:italic 48px/1 'Barlow Condensed';text-transform:uppercase}.hero span{display:block;margin-top:8px;color:#777;font-size:12px}
