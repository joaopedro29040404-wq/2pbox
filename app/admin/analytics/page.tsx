...
import { ArrowLeft, BarChart3, CheckCircle2, Eye, Funnel, MousePointerClick, ShoppingCart, Smartphone, Users, WalletCards } from 'lucide-react';
...
export default function AnalyticsPage() {
  ...
  const cards = data ? [['Visitantes', data.cards.visitors, Users], ['Visualizações de produtos', data.cards.productViews, Eye], ['Adições ao carrinho', data.cards.cartAdds, ShoppingCart], ['Checkouts iniciados', data.cards.checkouts, Funnel], ['Pagamentos iniciados', data.cards.payments, WalletCards], ['Pedidos aprovados', data.cards.approvedOrders, CheckCircle2], ['Faturamento', money(data.cards.revenue), BarChart3], ['Ticket médio', money(data.cards.averageTicket), WalletCards], ['Taxa de conversão', `${Number(data.cards.conversionRate || 0).toFixed(2).replace('.', ',')}%`, MousePointerClick]] : [];
    ...
    <main className="analytics-page">
      ...
      <section className="analytics-shell">
        ...
        {loading && !data ? <div className="analytics-loading">Carregando dados...</div> : data && <>
          <div className="analytics-cards">{cards.map(([label, value, Icon]: any) => <article className="analytics-card" key={label}><div><span>{label}</span><b><Icon size={17} /></b></div><strong>{value}</strong></article>)}</div>
          ...
          <section className="analytics-section"><div className="section-title"><div><p>LOCALIZAÇÃO</p><h2>De onde vêm os acessos</h2></div><span>estimativa por IP · cidade/região</span></div><div className="table-wrap"><table><thead><tr><th>Localização</th><th>Visitantes</th><th>Eventos</th></tr></thead><tbody>{(data.locations || []).map((row: any) => <tr key={row.location}><td><strong>{row.location}</strong></td><td>{row.visitors}</td><td>{row.events}</td></tr>)}</tbody></table>{(!data.locations || data.locations.length === 0) && <div className="analytics-empty">Ainda não foi possível estimar a localização dos acessos deste período.</div>}</div></section>
