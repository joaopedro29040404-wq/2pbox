...
type Product = { id: string; name: string; price: number; cost: number; active: boolean };
...
export default function MarketingPage() {
  ...
  async function load() {
    ...
    const [{ data: p, error: productsError }, { data: pr }, { data: c }] = await Promise.all([
      supabase.from('products').select('id,name,price,cost,active').eq('active', true).order('name').range(0, 999),
