...
export default function ProductsAdminPage() {
  ...
  async function load() {
    ...
    const [{ data: p, error: pe }, { data: c }] = await Promise.all([
      supabase.from('products').select('id,name,description,price,stock,active,category_id,image_url,images,slug').order('created_at', { ascending: false }),
  ...
  async function analyzeWithAI() {
    ...
    try {
      ...
      if (!data.copy) throw new Error('O Gemini não retornou o conteúdo do produto.');
      setAiCopy(data.copy);
  ...
  return <main className="admin-products-page">
    ...
    <section className="container admin-catalog">
      ...
      <div className="catalog-stats"><div><span>Produtos</span><strong>{products.length}</strong><small>Total cadastrado</small></div><div><span>Ativos</span><strong>{activeCount}</strong><small>Visíveis na loja</small></div><div><span>Estoque</span><strong>{stockCount}</strong><small>Unidades disponíveis</small></div><div><span>Inativos</span><strong>{inactiveCount}</strong><small>Fora da loja</small></div></div>

      {showForm && <form onSubmit={saveProduct} className="product-editor">
        <div className="editor-head"><div><p className="eyebrow">{editing ? 'EDIÇÃO' : 'NOVO CADASTRO'}</p><h2>{editing ? 'Editar produto' : 'Adicionar produto'}</h2><p>Use uma foto do produto para a IA criar um título e uma descrição precisos. A imagem não é modificada.</p></div><button type="button" className="icon-btn" onClick={cancel} aria-label="Fechar"><X size={20} /></button></div>

        <section className="ai-copy-box">
          <div className="ai-copy-heading"><div className="ai-icon"><Sparkles size={21} /></div><div><strong>Gerar título e descrição com IA</strong><span>Envie uma foto real do produto. O Gemini analisa a imagem, lê textos visíveis e preenche o cadastro sem gerar ou alterar nenhuma foto.</span></div></div>
          <div className="ai-copy-row">
            <label className="ai-copy-upload"><input type="file" accept="image/*" onChange={e => handleAiFile(e.target.files?.[0] || null)} /><Upload size={22} /><strong>{aiSource ? 'Foto selecionada' : 'Enviar foto do produto'}</strong><span>{aiSource ? aiSource.name : 'JPG, PNG ou WEBP · até 10 MB'}</span></label>
            {aiSource && <div className="ai-source-preview"><img src={URL.createObjectURL(aiSource)} alt="Foto enviada para análise" /><button type="button" onClick={resetAi} aria-label="Remover foto"><X size={16} /></button></div>}
          </div>
        ...
        <div className="editor-grid"><div className="editor-main">
          <label>Nome do produto<input required placeholder="Ex.: Carregador iPhone" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></label>
          <label>Descrição<textarea placeholder="Descreva o produto para seus clientes..." value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></label>
          <div className="editor-cols"><label>Preço<input required type="number" min="0" step="0.01" placeholder="0,00" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} /></label><label>Estoque<input required type="number" min="0" placeholder="0" value={form.stock} onChange={e => setForm({ ...form, stock: e.target.value })} /></label></div>
          ...
          <div className="media-head"><div><strong>Imagens do produto</strong><span>Até 8 fotos · a primeira é a principal</span></div><label className="upload-btn"><Upload size={16} /> Adicionar fotos<input type="file" accept="image/*" multiple hidden onChange={e => uploadImages(e.target.files)} /></label></div>
          {form.images.length ? <div className="editor-gallery">{form.images.map((src, i) => <div className="editor-thumb" key={src}><img src={src} alt="" /><div className="thumb-overlay"><button type="button" onClick={() => setMain(i)} disabled={i === 0}>{i === 0 ? <><Check size={12} /> Principal</> : 'Definir principal'}</button><button type="button" onClick={() => removeImage(i)} aria-label="Remover"><Trash2 size={14} /></button></div></div>)}</div> : <label className="dropzone"><ImageIcon size={30} /><strong>Adicione as fotos do produto</strong><span>PNG, JPG ou WEBP · até 5 MB por imagem</span><input type="file" accept="image/*" multiple hidden onChange={e => uploadImages(e.target.files)} /></label>}
        </div></div>
