...
export default function LojaPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!mounted) return;
      if (error) toast.error('Não foi possível carregar o catálogo', error.message); else setProducts((data ?? []) as unknown as Product[]);
  ...
  const productCategoryIds = (product: Product) => {
    const rows = product.product_categories ?? [];
  ...
  const productMatchesCategory = (product: Product) => {
    ...
    if (ids.includes(category)) return true;
    return Boolean(allCategories.find((c) => c.id === category && !c.parent_id && rows.some((row) => row.categories?.parent_id === category)));
  };
  ...
  }, [products, allCategories, category, search, sort]);
  const { page, setPage, totalPages, pageItems, from, to, total } = usePagination(filtered, PAGE_SIZE, `${category}|${search}|${sort}`);

  function selectCategory(id: string) {
    setCategory(id);
    const url = new URL(window.location.href);
    if (id === 'Todas') url.searchParams.delete('categoria'); else url.searchParams.set('categoria', id);
  ...
  return (
    <main className="loja-shell">
      ...
      <section className="container section loja-page" id="catalogo">
        ...
        <div className="catalog-controls"><TextField aria-label="Buscar produtos" placeholder="Buscar por nome ou descrição" value={search} icon={<Search size={17} />} onValueChange={setSearch} /><SelectField aria-label="Ordenar produtos" value={sort} options={SORTS} onValueChange={setSort} /></div>
        <div className="catalog-toolbar">
          ...
          <div className="catalog-count"><SlidersHorizontal size={15} /><span>{filtered.length} {filtered.length === 1 ? 'produto' : 'produtos'}</span></div>
        </div>
        {category !== 'Todas' && (
        ...
        {loading ? <SkeletonGrid count={PAGE_SIZE} height={340} /> : filtered.length === 0 ? (
          <div className="empty-catalog"><div className="empty-icon"><Search size={24} /></div><h3>Nenhum produto encontrado</h3><p>Tente outra busca ou escolha uma categoria diferente.</p><button type="button" className="primary" onClick={() => { setSearch(''); selectCategory('Todas'); }}>Ver todos os produtos</button></div>
        ) : (
          <><div className="product-grid ui-product-grid">{pageItems.map((product) => {
            ...
            return <article className="product ui-product-card" key={product.id}><div className="product-image-wrap"><Link href={`/produto/${product.slug}`} className="product-image-link"><div className="product-image ui-product-card-media"><ProductImage src={productCover(product)} alt={product.name} sizes="(max-width:700px) 50vw, 280px" />{product.stock > 0 && product.stock <= 5 && <span className="low-stock">Últimas unidades</span>}</div></Link><button type="button" aria-label={favorites.includes(product.id) ? 'Remover dos favoritos' : 'Adicionar aos favoritos'} className={`product-favorite ui-product-card-favorite ${favorites.includes(product.id) ? 'is-favorite' : ''}`} onClick={() => toggleFavorite(product.id)}><Heart size={16} strokeWidth={1.8} fill={favorites.includes(product.id) ? 'currentColor' : 'none'} /></button></div><div className="product-body ui-product-card-body"><Link href={`/produto/${product.slug}`} className="product-info-link ui-product-card-info"><small className="ui-product-card-eyebrow">{mainCategory}</small><h2 className="ui-product-card-title">{product.name}</h2>{product.description && <p className="ui-product-card-summary">{product.description}</p>}</Link><div className="product-buy ui-product-card-footer"><strong className="ui-product-card-price">R$ {Number(product.price).toFixed(2).replace('.', ',')}</strong><button type="button" className={`catalog-add-button ui-product-card-action ${added === product.id ? 'added' : ''}`} disabled={product.stock <= 0} onClick={() => addProduct(product)}>{added === product.id ? <><Check size={16} /> Adicionado</> : product.stock > 0 ? 'Adicionar' : 'Indisponível'}</button></div></div></article>;
