...
const products = [
  { name: 'Caderno Executivo 96 folhas', price: 'R$ 29,90', category: 'Papelaria', tag: 'Mais vendido' },
  { name: 'Fone Bluetooth Pro', price: 'R$ 89,90', category: 'Eletrônicos', tag: 'Destaque' },
  { name: 'Kit Canetas Premium', price: 'R$ 24,90', category: 'Papelaria', tag: 'Novo' },
  { name: 'Impressão Colorida A4', price: 'A partir de R$ 2,50', category: 'Impressão', tag: 'Serviço' },
];
  ...
  return (
    <main>
      ...
      <header className="header container">
        ...
        <nav><Link href="#categorias">Categorias</Link><Link href="#produtos">Produtos</Link><Link href="#contato">Contato</Link></nav>
      ...
      <section id="categorias" className="container section"><div className="section-head"><div><p className="eyebrow">ENCONTRE O QUE PRECISA</p><h2>Compre por categoria</h2></div><Link href="#produtos">Ver tudo <ArrowRight size={16}/></Link></div><div className="category-grid">{categories.map(({title,description,icon:Icon}) => <Link className="category" href="#produtos" key={title}><div className="category-icon"><Icon size={28}/></div><h3>{title}</h3><p>{description}</p><span>Explorar <ArrowRight size={15}/></span></Link>)}</div></section>

      <section id="produtos" className="container section"><div className="section-head"><div><p className="eyebrow">SELEÇÃO 2P BOX</p><h2>Produtos em destaque</h2></div><Link href="#produtos">Ver catálogo <ArrowRight size={16}/></Link></div><div className="product-grid">{products.map(p => <article className="product" key={p.name}><div className="product-image"><span>{p.category}</span><div className="product-placeholder">2P</div></div><div className="product-body"><small>{p.tag}</small><h3>{p.name}</h3><div className="rating"><Star size={14} fill="currentColor"/> 5,0</div><strong>{p.price}</strong><button>Adicionar ao carrinho</button></div></article>)}</div></section>
