...
export default function Home() {
  ...
  return (
    <main className="home-page">
      <SiteHeader />
      <section className={banners.length ? "home-hero home-carousel" : "home-hero home-carousel home-carousel-empty"} onTouchStart={event => { touchStartX.current = event.touches[0]?.clientX ?? null; }} onTouchEnd={event => { const start = touchStartX.current; const end = event.changedTouches[0]?.clientX ?? null; touchStartX.current = null; if (start === null || end === null || banners.length < 2) return; const distance = end - start; if (Math.abs(distance) < 45) return; setSlide(current => distance < 0 ? (current + 1) % banners.length : (current - 1 + banners.length) % banners.length); }}>
        {banners.length ? <div className="home-carousel-shell">
          <Link href={banners[slide]?.link_url || '/loja'} className="home-carousel-media" aria-label={'Abrir banner promocional ' + (slide + 1)}>
            ...
            <span className="home-carousel-vignette" aria-hidden="true" />
          </Link>
          <div className="home-carousel-topline" aria-hidden="true"><span>DESTAQUES 2P BOX</span><span>{String(slide + 1).padStart(2, '0')} / {String(banners.length).padStart(2, '0')}</span></div>
          {banners.length > 1 ? <>
            <button type="button" className="home-carousel-arrow left" onClick={(event)=>{event.preventDefault();event.stopPropagation();setSlide(current=>(current-1+banners.length)%banners.length)}} aria-label="Banner anterior"><ChevronLeft/></button>
            <button type="button" className="home-carousel-arrow right" onClick={(event)=>{event.preventDefault();event.stopPropagation();setSlide(current=>(current+1)%banners.length)}} aria-label="Próximo banner"><ChevronRight/></button>
            ...
            <div className="home-carousel-progress" aria-hidden="true"><span style={{width: ((slide + 1) / banners.length) * 100 + '%'}} /></div>
          </> : null}
        </div> : <div className="home-carousel-empty-content"><p className="home-eyebrow">2P BOX</p><h1>DESTAQUE EM<br/><em>CONSTRUÇÃO.</em></h1><p>Cadastre até 4 banners em Marketing para preencher este espaço.</p></div>}
      </section>
