...
export default function Home() {
  ...
  const [heroMode, setHeroMode] = useState<'logo' | 'carousel'>('logo');
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: categoryRows }, { data: productRows }, settings, { data: homeConfig }] = await Promise.all([
        ...
        getStoreSettings(),
        client.from('store_settings').select('home_hero_mode,home_banners,home_print_enabled,home_print_title,home_print_description,home_print_image_url').order('updated_at', { ascending: false }).limit(1).maybeSingle(),
      ]);
      if (!mounted) return;
      if (categoryRows) {
      ...
      setHeroMode(config?.home_hero_mode === 'carousel' && activeBanners.length ? 'carousel' : 'logo');
  ...
  return (
    <main className="home-page">
      <SiteHeader />
      {heroMode === 'carousel' && banners.length ? (
        <section className="home-hero home-carousel">
          <div className="home-carousel-media"><picture>{banners[slide]?.mobile_image_url ? <source media="(max-width:680px)" srcSet={banners[slide].mobile_image_url} /> : null}<img src={banners[slide].image_url} alt={banners[slide].title || 'Destaque 2P Box'} /></picture></div>
          <div className="home-carousel-overlay" />
          <div className="home-container home-carousel-content"><div><p className="home-eyebrow">2P BOX</p>{banners[slide]?.title ? <h1>{banners[slide].title}</h1> : null}{banners[slide]?.description ? <p className="home-lead">{banners[slide].description}</p> : null}{banners[slide]?.button_label && banners[slide]?.link_url ? <Link href={banners[slide].link_url || '/loja'} className="home-primary">{banners[slide].button_label} <ArrowRight size={18} /></Link> : null}</div></div>
          {banners.length > 1 ? <><button className="home-carousel-arrow left" onClick={() => setSlide((slide - 1 + banners.length) % banners.length)} aria-label="Banner anterior"><ChevronLeft /></button><button className="home-carousel-arrow right" onClick={() => setSlide((slide + 1) % banners.length)} aria-label="Próximo banner"><ChevronRight /></button><div className="home-carousel-dots">{banners.map((banner, index) => <button key={banner.id} className={index === slide ? 'active' : ''} onClick={() => setSlide(index)} aria-label={'Ir para banner ' + (index + 1)} />)}</div></> : null}
        </section>
      ) : (
      ...
      <section className="home-paths">
        <div className="home-container">
          <div className="home-section-head"><div><p className="home-eyebrow">ESCOLHA O QUE VOCÊ PRECISA</p><h2>DOIS CAMINHOS. UMA SÓ 2P BOX.</h2></div></div>
          <div className="home-path-grid">
            <Link href="/loja" className="home-path-card"><div className="home-path-icon"><ShoppingBag /></div><div><p>01 · LOJA</p><h3>PRODUTOS</h3><span>Encontre papelaria, eletrônicos, acessórios e variedades para o seu dia a dia.</span></div><ArrowRight className="home-path-arrow" /></Link>
            {printEnabled ? <Link href="/impressao" className="home-path-card print"><div className="home-path-icon"><Printer /></div><div><p>02 · SERVIÇO</p><h3>CENTRAL DE IMPRESSÃO</h3><span>Envie seus arquivos, escolha as configurações e faça seu pedido de impressão.</span></div><ArrowRight className="home-path-arrow" /></Link> : null}
          </div>
        </div>
      </section>

      {printEnabled ? <section className="home-print">
        <div className="home-container home-print-grid">
          <div><p className="home-eyebrow">CENTRAL DE IMPRESSÃO</p><h2>{printTitle}</h2><p>{printDescription}</p><div className="home-print-features"><span>✓ Preto e branco</span><span>✓ Colorido</span><span>✓ Frente e verso</span><span>✓ Papéis e serviços adicionais</span></div><Link href="/impressao" className="home-primary">COMEÇAR AGORA <ArrowRight size={18} /></Link></div>
          <div className="home-print-art">{printImage ? <img src={printImage} alt="Central de Impressão" /> : <div className="home-print-placeholder"><Printer size={58} /><strong>SEU ARQUIVO.<br /><b>NOSSA IMPRESSÃO.</b></strong></div>}</div>
        </div>
      </section> : null}
