...
export default function Home() {
  ...
  const [bannerViewportReady, setBannerViewportReady] = useState(false);
  ...
  useEffect(() => {
    ...
    const syncViewport = () => {
      setIsMobileBanner(media.matches);
      setBannerViewportReady(true);
    };
    syncViewport();
  ...
  useEffect(() => {
    ...
    let mounted = true;
    (async () => {
      const [{ data: categoryRows }, { data: productRows }, settings, { data: homeConfig }, { data: featuredConfig }] = await Promise.all([
  ...
  return (
    <main className="home-page">
            ...
            {loading || !bannerViewportReady ? <div className="home-carousel home-carousel-loading" aria-hidden="true"><div className="home-carousel-shell"><div className="home-carousel-skeleton"><span /></div></div></div> : banners.length ? <div className="home-carousel" onTouchStart={event => { touchStartX.current = event.touches[0]?.clientX ?? null; }} onTouchEnd={event => { const startX = touchStartX.current; const endX = event.changedTouches[0]?.clientX ?? null; touchStartX.current = null; if (startX === null || endX === null || banners.length < 2) return; const distance = endX - startX; if (Math.abs(distance) < 45) return; setSlide(current => distance < 0 ? (current + 1) % banners.length : (current - 1 + banners.length) % banners.length); }}>
              <div className="home-carousel-shell">
                ...
                <Link href={banners[slide]?.link_url || '/loja'} className="home-carousel-media" aria-label={'Abrir banner promocional ' + (slide + 1)}>
                  <Image src={bannerSource(banners[slide], isMobileBanner)} alt="Banner promocional 2P Box" className="home-carousel-image" width={1920} height={700} sizes="(max-width:680px) 100vw, 580px" priority={slide === 0} fetchPriority={slide === 0 ? 'high' : 'auto'} onLoad={() => setBannerImageLoaded(true)} />
                </Link>
                {banners.length > 1 ? (() => {
                  const nextBanner = banners[(slide + 1) % banners.length];
                  return (
                    <Image
                      src={bannerSource(nextBanner, isMobileBanner)}
                      alt=""
                      aria-hidden="true"
                      width={1920}
                      height={700}
      ...
      <section id="catalogo" className="home-section home-catalog">
        <div className="home-container">
          <div className="home-section-head"><div><p className="home-eyebrow">2P BOX</p><h2>DESTAQUES</h2></div><Link href="/loja" className="home-section-link">VER TODOS PRODUTOS <ArrowRight size={15} /></Link></div>
      ...
      <style jsx global>{`
        ...
        @media(max-width:680px){
          ...
          .home-category-section .home-section-head>div{
            min-width:0;
          }

          .home-category-section .home-eyebrow{
            margin-bottom:6px;
            font-size:7px;
            letter-spacing:.28em;
          }
          ...
          .home-category-section .home-section-link svg{
            width:12px;
            height:12px;
          }
          ...
          .home-category-section .home-category-grid::-webkit-scrollbar{
          ...
          .home-category-section .home-category-hint{
            display:none!important;
          }
        }
