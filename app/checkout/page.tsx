...
function CheckoutForm() {
  ...
  async function submitCheckout() {
    ...
    const { data, error } = await client.rpc('create_order_with_stock_v2', {
      p_customer_name: name.trim(), p_customer_phone: phone.trim(), p_customer_email: email.trim(), p_delivery_type: type,
      p_notes: notes.trim() || null, p_items: items.map(i => ({ id: i.id, quantity: i.quantity })), p_delivery_address: address,
    });
    ...
    if (type === 'pickup') {
      ...
      try {
        const payment = await fetch('/api/mercadopago/create-preference', {
          ...
          body: JSON.stringify({ orderId: id, total, email: email.trim(), name: name.trim(), cpf: cleanCpf, items: items.map(i => ({ id: i.id, name: i.name, price: i.price, quantity: i.quantity })) }),
        });
