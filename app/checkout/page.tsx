...
type PaymentResult = { id?: string | number; status?: string; statusDetail?: string };
...
function CheckoutForm() {
  ...
  const redirectToResult = useCallback((result: PaymentResult) => {
    clear();
    const statusValue = String(result.status || 'pending');
    window.location.href = `/pagamento/${encodeURIComponent(orderId)}?paymentId=${encodeURIComponent(String(result.id || ''))}&payment=${encodeURIComponent(statusValue)}`;
  }, [clear, orderId]);
  ...
  async function submitCheckout() {
    const client = supabase;
    if (!client || !items.length || submittingOrder) return;
    setStatus('');
    setPaymentError('');
    ...
    try {
      ...
      if (!user) {
        const { error: otpError } = await client.auth.signInWithOtp({ email: email.trim(), options: { emailRedirectTo: `${window.location.origin}/conta`, shouldCreateUser: true } });
  ...
  return <main>
    <div className="topbar">Finalizar pedido <span>•</span> 2P Box</div>
    <header className="header container checkout-header"><Link href="/" className="brand"><img className="brand-logo" src="/logo.pnh.png" alt="2P Box" /><div className="store-brand-copy"><strong>FINALIZAR PEDIDO</strong><small>INFORMAÇÕES PARA O SEU PEDIDO</small></div></Link></header>
    <section className="container section checkout-page">
      <Link href="/carrinho" className="secondary"><ArrowLeft size={16} /> Voltar ao carrinho</Link>
      <div className="checkout-intro"><p className="eyebrow">ÚLTIMA ETAPA</p><h1 className="checkout-title">Finalizar pedido</h1><p>{user ? 'Seus dados já estão preenchidos. Confira antes de confirmar.' : 'Compre sem cadastro e informe apenas seus dados para concluir o pedido.'}</p></div>
      <div className="checkout-form category">
        <div className="checkout-section"><div className="checkout-section-heading"><UserRound size={20} /><div><h2>Seus dados</h2><p>Usados para identificar e confirmar seu pedido</p></div></div><div className="form-grid"><label>Nome completo<input required placeholder="Digite seu nome" value={name} onChange={(e) => setName(e.target.value)} autoComplete="name" /></label><label>WhatsApp / telefone<input required placeholder="(11) 99999-9999" value={phone} onChange={(e) => setPhone(e.target.value)} inputMode="tel" autoComplete="tel" /></label><label>E-mail<input required type="email" placeholder="seuemail@email.com" value={email} onChange={(e) => setEmail(e.target.value)} disabled={!!user} autoComplete="email" /></label><label>CPF <span>(opcional)</span><input inputMode="numeric" placeholder="000.000.000-00" value={cpf} onChange={(e) => setCpf(e.target.value)} autoComplete="off" /></label></div>{!user && <div className="account-notice"><ShieldCheck size={18} /><span>Você não precisa criar senha para comprar. Depois do pedido, enviaremos um link de acesso ao seu e-mail.</span></div>}</div>
        <div className="checkout-section delivery-confirmation"><div className="checkout-section-heading"><Store size={20} /><div><h2>Forma de recebimento</h2><p>Escolhida no carrinho</p></div></div><div className="delivery-confirmed"><div className="delivery-confirmed-icon">{type === 'pickup' ? <Store size={22} /> : <MessageCircle size={22} />}</div><div><strong>{type === 'pickup' ? 'Retirada na loja' : 'Calcular frete pelo WhatsApp'}</strong><span>{type === 'pickup' ? 'Você conclui o pagamento pelo Mercado Pago nesta mesma página.' : 'O endereço é usado para calcular o frete e seguirá para o WhatsApp.'}</span></div><LockKeyhole size={16} /></div><Link href="/carrinho" className="change-delivery">Voltar ao carrinho para alterar a forma de recebimento</Link></div>
        {type === 'whatsapp_shipping' && <div className="checkout-section"><div className="checkout-section-heading"><MapPin size={20} /><div><h2>Endereço para o frete</h2><p>Preencha para enviarmos ao WhatsApp.</p></div></div><div className="form-grid address-grid"><label>CEP<input required inputMode="numeric" placeholder="00000-000" value={cep} onChange={(e) => setCep(e.target.value)} autoComplete="postal-code" /></label><label>Rua / avenida<input required placeholder="Nome da rua" value={street} onChange={(e) => setStreet(e.target.value)} autoComplete="street-address" /></label><label>Número<input required placeholder="123" value={number} onChange={(e) => setNumber(e.target.value)} inputMode="numeric" /></label><label>Complemento <span>(opcional)</span><input placeholder="Apto, bloco..." value={complement} onChange={(e) => setComplement(e.target.value)} /></label><label>Bairro<input required placeholder="Seu bairro" value={neighborhood} onChange={(e) => setNeighborhood(e.target.value)} autoComplete="address-level3" /></label><label>Cidade<input required placeholder="Sua cidade" value={city} onChange={(e) => setCity(e.target.value)} autoComplete="address-level2" /></label><label>Estado<input required maxLength={2} placeholder="SP" value={state} onChange={(e) => setState(e.target.value.toUpperCase())} autoComplete="address-level1" /></label></div><div className="shipping-whatsapp-note"><MessageCircle size={17} /><span>O pedido completo + endereço serão enviados para a 2P Box no WhatsApp.</span></div></div>}
        ...
        {paymentStarted && <div className="checkout-section payment-section"><div className="checkout-section-heading"><CreditCard size={20} /><div><h2>Pagamento seguro</h2><p>Escolha como pagar pelo Mercado Pago. O pedido já está reservado.</p></div></div><div className="payment-order-summary"><span>Pedido</span><strong>#{orderId.slice(0, 8).toUpperCase()}</strong><span>Total</span><strong>R$ {total.toFixed(2).replace('.', ',')}</strong></div><PaymentBrick amount={total} orderId={orderId} email={email} cpf={cpf} onResult={redirectToResult} onError={setPaymentError} />{paymentError && <div className="checkout-status payment-error">{paymentError}</div>}</div>}
        {!paymentStarted && status && <div className="checkout-status"><Loader2 size={17} />{status}</div>}
        {!paymentStarted && <div className="checkout-footer"><div><span>Total dos produtos</span><strong>R$ {total.toFixed(2).replace('.', ',')}</strong></div><button type="button" onClick={submitCheckout} className={`primary checkout-submit ${type === 'whatsapp_shipping' ? 'whatsapp-submit' : ''}`} disabled={!items.length || submittingOrder}>{items.length ? (type === 'whatsapp_shipping' ? 'Calcular frete pelo WhatsApp' : 'Continuar para pagamento') : 'Carrinho vazio'}</button></div>}
      </div>
    </section>
    <style jsx global>{`
      .delivery-confirmed{display:flex;align-items:center;gap:14px;padding:17px;border:1px solid #d9d9d9;border-radius:12px;background:#fafafa;color:#111}.delivery-confirmed-icon{width:42px;height:42px;flex:none;border-radius:9px;background:#ffc400;display:grid;place-items:center}.delivery-confirmed>div:nth-child(2){display:grid;gap:5px;flex:1}.delivery-confirmed strong{font-size:14px}.delivery-confirmed span{font-size:11px;color:#666;line-height:1.4}.delivery-confirmed>svg{color:#888;flex:none}.change-delivery{display:inline-block;margin-top:11px;font-size:11px;color:#777;text-decoration:underline}.account-notice{display:flex;align-items:flex-start;gap:10px;padding:13px 14px;margin-top:16px;background:#fff9d9;border:1px solid #f0d65b;border-radius:9px;color:#5c5000;font-size:11px;line-height:1.5}.account-notice svg{flex:none;color:#a47700;margin-top:1px}.address-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.shipping-whatsapp-note{display:flex;align-items:flex-start;gap:9px;margin-top:15px;padding:13px 14px;border-radius:9px;background:#f4f4f4;color:#555;font-size:11px;line-height:1.5}.shipping-whatsapp-note svg{flex:none;color:#111}.whatsapp-submit{background:#ffc400!important;color:#111!important}.payment-section{overflow:visible}.payment-section :global(.payment-brick-wrap){margin-top:8px}.payment-order-summary{display:grid;grid-template-columns:auto 1fr;gap:7px 14px;max-width:260px;margin:0 0 18px;padding:12px 14px;background:#fafafa;border:1px solid #eee;border-radius:10px;font-size:11px}.payment-order-summary span{color:#777}.payment-order-summary strong{text-align:right}.checkout-form{min-width:0}@media(max-width:600px){.address-grid{grid-template-columns:1fr}.checkout-section{min-width:0}.checkout-page{overflow-x:hidden}.checkout-footer{position:sticky;bottom:0;z-index:5;flex-direction:column;align-items:stretch}.checkout-footer>div{display:flex;justify-content:space-between;align-items:center}.checkout-submit{width:100%}.payment-order-summary{max-width:none}.payment-section{padding-bottom:20px}}
    `}</style>
  </main>;
}
