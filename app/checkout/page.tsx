...
function CheckoutForm(){
 ...
 async function submit(e:FormEvent){
  ...
  if(!user&&password.length<6){setStatus('Crie uma senha com pelo menos 6 caracteres.');return}
  setStatus(user?'Conferindo estoque e criando pedido...':'Criando sua conta e preparando o pedido...');
  ...
  if(!currentUser){
   ...
   if(error){setStatus(error.message.includes('already')?'Este e-mail já possui uma conta. Entre em /conta para acessar sua conta e depois finalize a compra.':error.message);return}
   if(!data.user){setStatus('Não foi possível criar sua conta. Tente novamente.');return}
   if(!data.session){setStatus('Sua conta foi criada. Confirme o e-mail para concluir a compra.');return}
   currentUser=data.user;setUser(currentUser);
  }else{
   const {error}=await supabase.auth.updateUser({data:{full_name:name.trim(),phone:phone.trim(),cpf:cpf.replace(/\D/g,'')||null}});
   if(error){setStatus(error.message);return}
  }
  const {data,error}=await supabase.rpc('create_order_with_stock',{p_customer_name:name.trim(),p_customer_phone:phone.trim(),p_customer_email:email.trim(),p_delivery_type:type,p_notes:notes.trim()||null,p_items:items.map(i=>({id:i.id,quantity:i.quantity}))});
  if(error){setStatus(error.message.replace(/^.*?: /,''));return}
  const id=data as string;setOrderId(id);const list=items.map(i=>`${i.quantity}x ${i.name} — R$ ${(i.price*i.quantity).toFixed(2).replace('.',',')}`).join('\n');clear();setDone(true);
