...
export default function PaymentResultPage() {
  ...
  const [status, setStatus] = useState<PaymentStatus>((searchParams.get('payment') as PaymentStatus) || 'pending');
  ...
  useEffect(() => {
    ...
    async function check() {
      try {
        ...
        if (!active) return;
        const next = ['approved', 'rejected', 'cancelled'].includes(data.paymentStatus) ? data.paymentStatus as PaymentStatus : 'pending';
        setStatus(next);
  ...
  return (
    <main className="payment-result-page">
      ...
      <section className={`payment-result-card ${status}`}>
        ...
        {!loading && status === 'pending' && <div className="checking"><RefreshCw size={15}/> Aguardando confirmação automática</div>}
