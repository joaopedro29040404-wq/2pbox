...
import { CheckCircle2, XCircle, Clock3, Loader2, ArrowRight, PackageSearch } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    async function check() {
      try {
        ...
        setLoading(false);
        if (!['approved', 'rejected', 'cancelled'].includes(data.paymentStatus)) timer = window.setTimeout(check, 2500);
      } catch {
        if (active) { setLoading(false); timer = window.setTimeout(check, 4000); }
      }
    }
    check();
