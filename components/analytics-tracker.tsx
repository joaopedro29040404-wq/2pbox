...
function getAttribution(): Attribution {
  try {
    ...
    const next = saved || current.source || current.medium || current.campaign || current.referrer ? (saved || current) : current;
    if (!saved && (current.source || current.medium || current.campaign || current.referrer)) localStorage.setItem(ATTRIBUTION_KEY, JSON.stringify(next));
    return next;
  } catch {
...
function readCart(): CartItem[] {
  try {
    ...
    return Array.isArray(parsed) ? parsed.map((item) => ({ id: String(item.id), name: String(item.name || ''), price: Number(item.price || 0), quantity: Number(item.quantity || 0) })) : [];
  } catch {
    return [];
  }
}
...
export default function AnalyticsTracker() {
  ...
  useEffect(() => {
    ...
    async function send(payload: EventPayload) {
      ...
      try {
        await originalFetch('/api/analytics/event', {
          method: 'POST',
    ...
    if (currentPath === '/checkout') {
      ...
      if (items.length) {
        ...
        try {
          if (sessionStorage.getItem(LAST_CHECKOUT_KEY) !== marker) {
            sessionStorage.setItem(LAST_CHECKOUT_KEY, marker);
            void send({ event_name: 'begin_checkout', page_path: currentPath, value: cartTotal(items) });
          }
        } catch {
          void send({ event_name: 'begin_checkout', page_path: currentPath, value: cartTotal(items) });
        }
    ...
    const interval = window.setInterval(() => {
      ...
      for (const item of nextCart) {
        ...
        if (!previous || item.quantity > previous.quantity) {
          void send({ event_name: 'add_to_cart', product_id: item.id, value: cartTotal(nextCart), metadata: { quantity: Math.max(1, item.quantity - (previous?.quantity || 0)), name: item.name } });
        }
      }
      for (const item of lastCart) {
        const current = after.get(item.id);
        if (!current || current.quantity < item.quantity) {
          void send({ event_name: 'remove_from_cart', product_id: item.id, value: cartTotal(nextCart), metadata: { quantity: item.quantity - (current?.quantity || 0), name: item.name } });
        }
      }
      lastCart = nextCart;
    }, 500);
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes('/api/mercadopago/create-payment')) {
        void send({ event_name: 'payment_started', page_path: window.location.pathname, value: Number((init?.body as string | undefined) ? 0 : 0), metadata: { endpoint: '/api/mercadopago/create-payment' } });
      }
    ...
    const orderAttributionInterval = window.setInterval(() => {
      try {
        ...
        const attribution = getAttribution();
        void originalFetch('/api/analytics/order-attribution', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ order_id: orderId, session_id: getSessionId(), utm_source: attribution.source, utm_medium: attribution.medium, utm_campaign: attribution.campaign }),
          keepalive: true,
        }).catch(() => undefined);
      } catch {}
    }, 500);
    ...
    return () => {
      ...
      window.clearInterval(orderAttributionInterval);
