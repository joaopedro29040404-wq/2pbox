-- Checkout seguro: cria o pedido, grava os itens e baixa o estoque em uma única transação.
...
begin
  ...
  -- Bloqueia as linhas dos produtos durante o checkout para evitar venda acima do estoque.
  ...
  insert into public.orders (customer_name, customer_phone, customer_email, delivery_type, notes, total)
  values (trim(p_customer_name), trim(p_customer_phone), nullif(trim(p_customer_email), ''), p_delivery_type, nullif(trim(p_notes), ''), v_total)
