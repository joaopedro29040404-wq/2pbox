-- Libera a modalidade de envio imediato no banco para instalações
-- onde a migration original de modalidades já havia sido aplicada.
alter table public.orders
  drop constraint if exists orders_delivery_type_check;

alter table public.orders
  add constraint orders_delivery_type_check
  check (delivery_type in ('pickup','whatsapp_shipping','own_delivery','express_delivery','app_delivery'));
