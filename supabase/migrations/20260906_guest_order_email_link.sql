-- 2P Box | Vinculação de pedidos de convidados à conta pelo e-mail
-- Permite que uma compra feita sem login apareça na conta quando o cliente
-- entrar usando o mesmo e-mail informado no checkout.

-- Pedidos feitos enquanto o cliente estava autenticado continuam sendo
-- vinculados pelo customer_id através do trigger existente.
-- Para pedidos de convidado, a conta autenticada pode enxergar somente
-- pedidos cujo e-mail coincide exatamente com o e-mail da sessão.
drop policy if exists "Customers can read own orders by email" on public.orders;
create policy "Customers can read own orders by email"
...
-- Quando o cliente estiver autenticado no checkout, garante também o
-- customer_id mesmo que o pedido tenha sido criado por uma função SECURITY DEFINER.
...
begin
  ...
  values (v_customer_id, trim(p_customer_name), trim(p_customer_phone), nullif(trim(p_customer_email), ''), p_delivery_type, case when p_delivery_type='whatsapp_shipping' then p_delivery_address else null end, nullif(trim(p_notes), ''), v_total)
