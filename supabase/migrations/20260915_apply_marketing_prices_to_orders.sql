-- Marketing V2: aplica o preço promocional no fechamento do pedido.
-- Não altera Mercado Pago. O checkout continua chamando a mesma função RPC;
-- apenas o cálculo server-side do valor dos produtos passa a considerar promoções ativas.
...
begin
  ...
  if p_delivery_type not in ('pickup','whatsapp_shipping','own_delivery','express_delivery','app_delivery') then
  ...
  -- Bloqueia cada produto e calcula o preço efetivo no servidor.
  -- O cliente nunca consegue impor um preço menor através do payload.
  ...
  loop
    ...
    select promotional_price into v_promotion
    from public.promotions
    where product_id = v_product.id
      and active = true
      and starts_at <= now()
      and ends_at >= now()
      and promotional_price < v_product.price
    order by promotional_price asc
    limit 1;

    if found then
      v_unit_price := v_promotion.promotional_price;
    end if;
